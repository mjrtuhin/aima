...
def upgrade() -> None:
    ...
    try:
        op.execute("SELECT create_hypertable('orders', 'created_at', if_not_exists => TRUE, migrate_data => TRUE)")
    except Exception:
        pass
    ...
    try:
        op.execute("SELECT create_hypertable('customer_events', 'created_at', if_not_exists => TRUE, migrate_data => TRUE)")
    except Exception:
        pass
    ...
    try:
        op.execute("SELECT create_hypertable('brand_mentions', 'created_at', if_not_exists => TRUE, migrate_data => TRUE)")
    except Exception:
        pass
